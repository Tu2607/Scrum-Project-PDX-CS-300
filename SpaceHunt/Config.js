...
function updateConfig(space, ship, cheat, artifactSet, visitedPoints) {
	...
	ship = new Ship(8 * eval(config.xout.value), 8 * (eval(config.mapSize.value) - eval(config.yout.value)), eval(config.energy.value), eval(config.supplies.value), eval(config.credits.value), false, false);
